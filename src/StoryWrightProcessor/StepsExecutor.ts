...
export class StepsExecutor {
  public static async executesteps(
    ...
    if (
      steps[0]["type"] !== StepType.SaveScreenshot &&
      steps[0]["type"] !== StepType.CropScreenshot
    ) {
    ...
    for (const step of steps) {
      ...
      switch (step["type"]) {
        ...
        case StepType.CropScreenshot: {
          await executor.elementScreenshot(step.locator.value, testName);
          break;
        }
        ...
        case StepType.SetElementText: {
          await executor.setElementText(step.locator.value, step.text);
          break;
        }
        case StepType.SendKeys: {
          ...
          if (step.keys === "") {
            ...
            if (!keyToSend) {
              await executor.setElementText(step.locator.value, step.keys);
            } else {
